...
function update_index() {
    ...
    while (i < length) {
        if (rray.indexOf(LINK_DATA[i].source.id) == -1) {
            ...
            single_source[count_single_source] = LINK_DATA[i].source.id
        ...
        count1 += 1
        var i_plus = false
        if (countOccurences(tar, LINK_DATA[i].source.id) > 1 || countOccurences(sor, LINK_DATA[i].target.id) > 1) {
