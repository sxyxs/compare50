...
function init_index() { //创建一个表格，有#	Submissions	Score 作为title，里面没填东西但是架子搭起来了，靠update_index填数据
    ...
    thead_tr.append("td")
        .attr("scope", "col")//col 规定单元格是列的表头
        .text("#");
    thead_tr.append("td")
        .attr("scope", "col")
        .attr("colspan", 2) //规定单元格应该横跨的列数。这里是2格
        .text("Submissions");
    thead_tr.append("td")
        .attr("scope", "col")
        .text("Score");
...
function update_index() {
    ...
    let table_data = INDEX.selectAll("tr").data(LINK_DATA, d => d.index);// create a empty but same structure as graph
    ...
    new_trs.append("th")
        .attr("scope", "row")
        .text(d => "group " + (single_source.indexOf(d.source.id)+1)); 
