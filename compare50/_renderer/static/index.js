...
function update_index() {
    ...
    var count1 = 0;
    var count2 = 0
    var count_single_source = 0
    var count_single_target = 0
    var count_single_source_occor_time = 0
    let table_data = INDEX.selectAll("tr").data(LINK_DATA, d => d.index);
    var tar = new Array() // store all the value show in target
    var sor = new Array() // store all the value show in source
    var single_source = new Array() // store all the value show in source once and that value not show in target
    var single_target = new Array() // store all the value show in target once and that value not show in source
    var single_source_occor_time = new Array() // store all the value show in source after remove the source which not list in single_source
    var length = LINK_DATA.length
    var i = 0
    var j = 0
    const countOccurences = (arr, value) => arr.reduce((a, v) => v === value ? a + 1 : a + 0, 0); // count the number of times a same value appears in array

    var rray = new Array() // store all the value show in both source and target once
    while (i < length) { // this loop is to store the correspond value for each array
        if (rray.indexOf(LINK_DATA[i].source.id) == -1) {
            rray[count2] = LINK_DATA[i].source.id
            single_source[count_single_source] = LINK_DATA[i].source.id //it will store the source which do not list in target.id
            count_single_source += 1
            count2 += 1
        }
        if (rray.indexOf(LINK_DATA[i].target.id) == -1) {
            rray[count2] = LINK_DATA[i].target.id
            single_target[count_single_target] = LINK_DATA[i].target.id
            count_single_target += 1
            count2 += 1
        }
        sor[count] = LINK_DATA[i].source.id
        count += 1
    ...
    while (i < length) { // remove all the link_data which the source don't show in single_source
        if (single_source.indexOf(LINK_DATA[i].source.id) == -1) { // 
            ...
            single_source_occor_time[count_single_source_occor_time] = LINK_DATA[i].source.id
            count_single_source_occor_time += 1
            i += 1
        }
    }
    
    //console.log(countOccurences(single_source_occor_time, 'tests/files/sub_a/2.cpp'))
    for(i=0;i<length;i++){
        
       // console.log(single_source_occor_time.indexOf(LINK_DATA[i].source.id) )
    }
    
    i = 0
    while (i < length) {
        if (single_target.indexOf(LINK_DATA[i].target.id) !== -1) { 
            single_target.splice(single_target.indexOf(LINK_DATA[i].target.id), 1)
            i += 1
        }
        else if (single_target.indexOf(LINK_DATA[i].target.id) == -1) {
            if (countOccurences(single_source_occor_time, LINK_DATA[i].source.id) > 1) {//if remove, source will dispare. Then keep that
                single_source_occor_time.splice(single_source_occor_time.indexOf(LINK_DATA[i].source.id),1)
                LINK_DATA.splice(i, 1)
                length = length - 1
            }
            else {
    ...
    /*while (i < length) {
        i_plus = false
        if(test.indexOf(LINK_DATA[i].target.id) == -1){
            test.push(LINK_DATA[i].target.id)
            i_plus = true
        }
        else{
            LINK_DATA.splice(i, 1)
    ...
    for (i = 0; i < single_source.length; i++) 
    {
        ...
        for (j = 0; j < length; j++) 
        {
            ...
            console.log("J")
            console.log(j)
            if (LINK_DATA[j].source.id == single_source[i]&&start_order == false) {//当碰到了第一个和这个分组一样的 source id
                console.log("aba")
            ...
            if(need_delete == true && start_index == j){
                LINK_DATA.splice(j+1, 1)
            }
            else if(need_delete == true && start_index >= j){
